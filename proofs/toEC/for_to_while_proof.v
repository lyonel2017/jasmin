...
Proof.
  ...
  rewrite (ziotaS_cons n1); first lia.
  rewrite wrange_UpTo_ziota.
...
Proof.
  ...
  have -> : n2 - n1 = Z.succ (n2 - n1 - 1) by ring.
  rewrite (ziotaS_cons (- n2)); first lia.
  rewrite wrange_DownTo_ziota /=.
...
Proof using E E0 X asm_op dc ep ev for_to_while_ok fresh_var_ident p p' rE0
  ...
  - by case: hst.
  - move=> y hy.
    rewrite Vm.setP_neq; last by case: hst => _ _ /(_ y hy).
...
Proof using E E0 X asm_op dc ep ev for_to_while_ok fresh_var_ident p p' rE0
  ...
  - split=>//=.
    ...
    + by case: hst.
    move=> w hw.
    rewrite Vm.setP_neq; last by case: hst => _ _ /(_ w hw).
    ...
    have hround : forall m n1, m = Z.to_nat (vhi0 - n1) ->
      ...
      - rewrite /iwrite_var.
        ...
        - split=> //=.
          ...
          rewrite Vm.setP_neq; first by rewrite eq_sym.
          rewrite Vm.setP_neq; first by rewrite eq_sym.
      ...
      have hst04 : st_eq_on X r0 r4.
        ...
        - by case: hst03.
        move=> y hy; rewrite Vm.setP_neq; last by case: hst03 => _ _ /(_ y hy).
